...
func NewKVBoltDBBackend(filename string, bucketName string, maxKeysPerBucket int) *KVBoltDBBackend {
	var err error
	b := KVBoltDBBackend{filename, bucketName, nil, nil, maxKeysPerBucket}
...
// TODO: add bloom filter to Put and Get - counting bloomfilter http://godoc.org/github.com/pmylund/go-bloom
// TODO: memcached ops
// https://github.com/memcached/memcached/blob/master/doc/protocol.txt
// "set", "add", "replace", "append" or "prepend"
// "get", "gets"
// "touch", "stats", "stats <args>"
// "delete", "incr", "decr"
// (strict behavior for memcached: item must exists and must be integer for incr/decr, item wont be automatically created w/ value 0 as redis)
// parameters:
// <key> <flags> <exptime> <bytes> <cas unique> [noreply]
// append/prepend do not honor flags + exptime
// store data
...
func (be KVBoltDBBackend) Increment(key []byte, value int, create_if_not_exists bool) (int, error) {
	...
	err := be.db.Update(func(tx *bolt.Tx) error {
		...
		if be.bloomFilter[be.bucketName].Test(key) == false {
			...
			i = i + value
			err = bucket.Put(key, []byte(string(i)))
			if err != nil {
				return fmt.Errorf("Error storing incr/decr value for key %s - %d", string(key), i)
			}
			ret = i
...
func main() {
	var vdb = NewKVBoltDBBackend("bolt.db", "memcached", 10000)
	defer vdb.CloseDB()
	...
	log.Printf("Get key (should yield no value)")
	w, err = vdb.Get(key)
	...
	if w == nil {
		...
		log.Println(string(w))
	}
	log.Printf("Set int value for key")
	err = vdb.Set(key, []byte("1"))

	if err != nil {
		log.Fatal(err)
	}

	log.Printf("Incr by 1")
	v, err := vdb.Incr(key, 1)

	if err != nil {
		log.Fatal(err)
	}
	log.Printf("Value from Incr %d", v)

	log.Printf("Get key after incr")
	w, err = vdb.Get(key)
	...
	log.Printf("Replace key")
	err = vdb.Replace(key, value)

	if err != nil {
		log.Fatal(err)
	}

	log.Printf("Add key (should fail)")
	err = vdb.Add(key, value)
